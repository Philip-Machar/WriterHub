  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        {/* Completed Work Notification for Posters */}
        {userRole === "poster" && (() => {
          const completedGigs = gigs.filter(gig => gig.status === "approved" && gig.completedWork && gig.postedBy === auth.currentUser?.uid);
          if (completedGigs.length > 0) {
            return (
              <div className="mb-6 glass-panel rounded-3xl p-4 bg-gradient-to-r from-emerald-500/10 to-green-500/10 border border-emerald-400/20">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-r from-emerald-500/30 to-green-500/30 flex items-center justify-center border border-emerald-400/30">
                    ...
                    <div>
                      <p className="text-emerald-200 font-semibold">
                        You have {completedGigs.length} completed work{completedGigs.length > 1 ? 's' : ''} ready for download!
                      </p>
                      <p className="text-emerald-100 text-sm">
                        Visit your poster panel to access your completed work.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => navigate("/poster")}
                    className="glass-button bg-gradient-to-r from-emerald-500/20 to-green-500/20 hover:from-emerald-500/30 hover:to-green-500/30 text-emerald-200 px-4 py-2 rounded-xl font-semibold transition-all duration-300 hover:scale-105 border border-emerald-400/30"
                  >
                    View Work
                  </button>
                </div>
